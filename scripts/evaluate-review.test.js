...
import { determineReviewState, parseMetadataBlock, evaluateFeedbackFallback, submitPRReview, sys, getEnvConfig, fetchGithubApi, fetchBotFeedback, dismissPreviousChangesRequested, main } from './evaluate-review.js';
...
describe('submitPRReview', () => {
  ...
  it('should throw an error for invalid PR numbers', () => {
    ...
    expect(() => submitPRReview('5; rm -rf /', 'APPROVE', 'Looks good')).toThrow('Invalid PR number: 5; rm -rf /');
    expect(execSyncSpy).not.toHaveBeenCalled();
  });

  it('should escape shell metacharacters in the message', () => {
    submitPRReview('42', 'COMMENT', 'Hello `world` $HOME "test"');
    expect(execSyncSpy).toHaveBeenCalledWith('gh pr review 42 --comment -b "Hello \\`world\\` \\$HOME \\"test\\""');
  });
...
describe('getEnvConfig', () => {
  ...
  it('should throw an error when environment variables are missing', () => {
    ...
    expect(() => getEnvConfig()).toThrow('Missing PR_NUMBER or GITHUB_REPOSITORY environment variables.');
  });
});
...
describe('fetchGithubApi', () => {
  ...
  it('should throw a wrapped error when gh api fails', () => {
    ...
    expect(() => fetchGithubApi('bad/endpoint')).toThrow("Failed to fetch GitHub API endpoint 'bad/endpoint': network error");
  });
});
    ...
    execSyncSpy = vi.spyOn(sys, 'execSync').mockImplementation((cmd) => {
      if (cmd.includes('/issues/')) {
        return Buffer.from(JSON.stringify([
          { body: 'issue comment', created_at: '2024-01-02T00:00:00Z', user: { login: 'github-actions[bot]' } },
          { body: 'ignored', created_at: '2024-01-01T00:00:00Z', user: { login: 'some-user' } }
        ]));
      }
      if (cmd.includes('/pulls/') && cmd.includes('/comments')) {
        return Buffer.from(JSON.stringify([
          { body: 'review comment', created_at: '2024-01-03T00:00:00Z', user: { login: 'opencode-bot' } }
        ]));
      }
      if (cmd.includes('/reviews')) {
        return Buffer.from(JSON.stringify([
          { body: 'review body', submitted_at: '2024-01-04T00:00:00Z', user: { login: 'opencode-bot' } }
        ]));
      }
      return Buffer.from('[]');
    });
...
describe('dismissPreviousChangesRequested', () => {
  ...
  it('should dismiss previous bot CHANGES_REQUESTED reviews', () => {
    execSyncSpy.mockImplementationOnce(() => Buffer.from(JSON.stringify([
  ...
  it('should warn and not throw when dismissing fails', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to dismiss previous reviews'));
  });
});
...
describe('main', () => {
  ...
  it('should exit cleanly when no bot feedback is found', () => {
    main();
    expect(consoleWarnSpy).toHaveBeenCalledWith('No feedback from bot found. Exiting review evaluation.');
  ...
  it('should submit an APPROVE review when no issues found', () => {
    execSyncSpy.mockImplementation((cmd) => {
      if (cmd.includes('/issues/')) {
        return Buffer.from(JSON.stringify([
          { body: 'LGTM', created_at: '2024-01-01T00:00:00Z', user: { login: 'github-actions[bot]' } }
        ]));
      }
      if (cmd.includes('/pulls/') && cmd.includes('/comments')) {
        return Buffer.from('[]');
    ...
    main();
    expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Successfully submitted PR review status!'));
    expect(exitSpy).not.toHaveBeenCalled();
  });
  ...
  it('should exit with code 1 on unhandled errors', () => {
    delete process.env.PR_NUMBER;
    main();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Execution failed:', 'Missing PR_NUMBER or GITHUB_REPOSITORY environment variables.');
